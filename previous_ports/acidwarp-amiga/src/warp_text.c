...
/* Amiga - */
#ifdef AMIGA
        "\nr <n> = Use other modeid. Hexadecimal or decimal value OK.\n",
        "\ne     = Enforce ECS compatible options.\n",
        "\nl     = Effect generation sub-task has lower priority.\n",
        "\nf <n> = Which effect shuld go as the first(0-40)?\n",
        "\np <n> = Which palette should be the leading one(0-8)?\n",
        "\nm     = Use Morten Eriksen C2P routine(good on ECS/AGA)\n",
#else
...
/* Amiga - */
        ...
        "      must be a modeid value expressed as a hexadecimal or decimal. By default\n",
        "      Workbench modeid will be used if it is opened and if not then 0 which\n",
        "      maps into something near i guess ;-) .\n",
        "\n",
        "'e' - Allows to enforce ECS compatible options(modeid of at least 6bits!).\n",
        "\n",
        "'l' - Make the effect generation sub-task lower on priority than the main\n",
        "      task(useful on slow machines and with super fast warping).\n",
        "\n",
        "'f' - This option allows to choose the very first effect(0-40).\n",
        "\n",
        "'p' - This option sets the default palette(0-8) for the entire show.\n",
        "\n",
        "'m' - This option causes C2P by Morten Eriksen to be used, so rendering on\n",
        "      ECS or AGA is actually much faster!\n",
        "\n",
#else
        "'r' - An option to use higher resolutions has been implemented. This option\n",
